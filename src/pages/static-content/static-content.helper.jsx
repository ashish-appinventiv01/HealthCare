...
const LegalContent = lazy(() => import('./commonLegalContent/LegalContent.jsx'))

export function createLegalPage(pageKey) {
  return function LegalPage() {
    return <LegalContent {...LEGAL_COPY[pageKey]} />
  }
}

export function normalizeStaticPage(page) {
  return String(page || '')
    .toLowerCase()
    .replace(/^legal-/, '')
}

export function getStaticRenderConfig(page) {
  const key = normalizeStaticPage(page)
  if (key === 'faqs') return { type: 'faqs' }
  if (key === 'terms' || key === 'privacy') return { type: 'legal', legalKey: key }
  return { type: 'not_found' }
}

// Hook-style API to mirror other helpers (e.g., dashboard.helper)
export default function useStaticContentHelper() {
  return {
    LEGAL_COPY,
    createLegalPage,
    normalizeStaticPage,
